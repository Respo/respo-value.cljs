...
(def comp-nil
  (create-comp :number (fn [] (fn [state mutate!] (comp-text "nil" widget/literal)))))

(def comp-string
  (create-comp :string (fn [x] (fn [state mutate!] (comp-text (pr-str x) widget/literal)))))

(def comp-function
  (create-comp :function (fn [] (fn [state mutate!] (comp-text "fn" widget/literal)))))

(def comp-keyword
  (create-comp :keyword (fn [x] (fn [state mutate!] (comp-text (str x) widget/literal)))))

(defn toggle-folding [mutate!] (fn [e dispatch!] (mutate!)))

(def comp-number
  (create-comp :number (fn [x] (fn [state mutate!] (comp-text (str x) widget/literal)))))

(def comp-bool
  (create-comp :bool (fn [x] (fn [state mutate!] (comp-text (str x) widget/literal)))))
         ...
         (fn [index field]
           [index
            (div
             ...
             (render-value (last field) (inc level)))])))))

(defn render-children [xs level]
  (div
   {:style (merge widget/style-children layout/column)}
   (->> xs (map-indexed (fn [index child] [index (render-value child (inc level))])))))
